...
class AdminFileController extends Base
{
    protected $filetype = [
    ...
    {
        $id     = $this->id;
        $result = Attachment::destroy(function ($query) use ($id) {
            $query->whereIn('id', $id);
        });
        if ($result) {
    ...
    {
        $admin_file = Attachment::get($this->id);
        if (!$admin_file) {
    ...
    {
        $data = Attachment::onlyTrashed()->whereIn('id', $this->id)->select();
        if ($data) {
            foreach ($data as $d) {
