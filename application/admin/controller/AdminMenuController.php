...
class AdminMenuController extends Base
{
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $menu = AdminMenu::create($this->param);
            if ($menu) {
                return $this->success();
            }
    ...
    public function del()
    {
        ...
        $admin_menu_son = AdminMenu::get(function ($query) use ($id) {
            if(is_array($id)){
        ...
        $id = $this->id;
        $result = AdminMenu::destroy(function ($query) use ($id) {
            $query->whereIn('id', $id);
        });
        if ($result) {
